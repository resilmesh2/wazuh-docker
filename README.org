...
The next step is to run Wazuh using the original =docker-compose-original.yaml=
file, in order to generate all the config files.  You won't need this Wazuh
instance, so take it down afterwards.
